...
is removed, text is zeroed, or text is shuffled. These findings support a return-conservative
modeling principle for disclosure events: use filing text to transport risk, liquidity, and
uncertainty responses while preserving abnormal-return ranking in the no-event market state.
\end{abstract}
...
\paragraph{The no-event baseline is not the RC-DJB anchor.}
...
RC-DJB instead trains its no-event state jointly with a disclosure bridge, control penalty,
likelihood term, and rank objective. The bridge absorbs event-response variation in volatility,
volume, and uncertainty, while the return-mean firewall prevents high-dimensional text from directly
rewriting the abnormal-return mean. The empirical claim is therefore not that an isolated no-event
forecaster is sufficient; it is that a jointly trained no-event return anchor is useful when
disclosure text is routed through response-distribution transport.
...
This tradeoff is the main empirical evidence for the return-mean firewall.
...
isolates the routing architecture rather than encoder scale. Rank IC is statistically positive but
economically small; net performance after spreads, commissions, market impact, borrow constraints,
capacity, and turnover requires a separate portfolio-construction study
\citep{grinold2000active,novymarx2016taxonomy,frazzini2018trading,lopezdeprado2018advances}. The
return-mean firewall is an architectural and predictive constraint, not structural causal identification
\citep{rubin1974estimating,imbens2015causal}.
...
text directly move the abnormal-return mean is not required for strong response modeling. A
return-conservative distributional bridge obtains positive held-out rank IC while still improving
multi-target event-response MSE over concat fusion and DOT. The evidence supports a more precise
architecture principle for disclosure modeling: use text to
